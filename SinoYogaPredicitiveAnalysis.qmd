...
6.  We saved this new dataset as **msdm_cleandata.csv** to use in Rstudio to clean, transform, wrangle , and visualzie in predictive analysis.
...
We cleaned the dataset **msdm_data** by upper_camel the varaibale names and also removing any spaces and special characters. Afterwards we renamed that dataset as **SurveyData** to keep seperate. We changed the **RecordedDate** variable to Date format and checked to make sure it was changed.
...
We created a correlation matrix to see what variables were positively and negatively impacting **WtpDollars (willingness to pay).** This correlation was extemely cluttered so we focused on variables that seemed to correlate with **WtpDollars.**
